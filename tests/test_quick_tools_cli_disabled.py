...
def test_quick_tools_cli_entrypoints_disabled():
    """Ensure quick-tool wrappers handle CLI calls appropriately.

    For each quick_tools module, if it exposes `execute_from_cli`, calling it
    with no args should either:
    - Raise NotImplementedError or ImportError (menu-only tools)
    - Raise SystemExit (tools with working CLI that require arguments)
    """
    for modname in iter_quick_tool_modules():
        ...
        if callable(cli):
            try:
                ...
                # Some tools now have working CLI implementations
                continue
            else:
                raise AssertionError(
                    f"Module {modname}.execute_from_cli did not raise expected exception"
                )
