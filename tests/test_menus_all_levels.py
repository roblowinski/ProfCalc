...
def test_annual_monitoring_and_submenus(monkeypatch):
    ...
    monkeypatch.setattr(
        ms, "profcalc_profcalc_menu", lambda: calls.setdefault("profcalc", True)
    )
    monkeypatch.setattr(
        ms, "shoreline_analysis_menu", lambda: calls.setdefault("shoreline", True)
    )
...
def test_profcalc_profile_menus(monkeypatch):
    ...
    monkeypatch.setattr(
        ms, "survey_vs_design_menu", lambda: calls.setdefault("design", True)
    )
    monkeypatch.setattr(
        ms, "survey_vs_survey_menu", lambda: calls.setdefault("survey", True)
    )
...
def test_quick_tools_and_conversion_submenus(monkeypatch):
    ...
    monkeypatch.setitem(
        __import__("sys").modules, "profcalc.cli.quick_tools.fix_bmap", fake_fix
    )
    monkeypatch.setitem(
        __import__("sys").modules, "profcalc.cli.quick_tools.bounds", fake_bounds
    )
    monkeypatch.setitem(
        __import__("sys").modules, "profcalc.cli.quick_tools.inventory", fake_inventory
    )
    monkeypatch.setitem(
        __import__("sys").modules, "profcalc.cli.quick_tools.assign", fake_assign
    )
    monkeypatch.setitem(
        __import__("sys").modules,
        "profcalc.cli.quick_tools.get_profile_dates",
        fake_get_dates,
    )
    ...
    monkeypatch.setitem(
        __import__("sys").modules, "profcalc.cli.quick_tools.convert", fake_convert
    )
