...
New: run the package directly or use the installed CLI
----------------------------------------------------

You can now run the interactive menu in a few convenient ways without
manually managing PYTHONPATH:

- If you'd like to run the package directly from the repo use:

```powershell
python -m profcalc
```

- If you installed the package in editable mode (`pip install -e .`) a
  console command `profcalc` will be available (preferred):

```powershell
profcalc
```

- For convenience two small helper launchers are included in the repo
  root that pick the installed command if present and otherwise fall
  back to `python -m profcalc`:

```powershell
.\run_profcalc.ps1  # PowerShell helper (prefers installed 'profcalc')
.\run_profcalc.cmd  # CMD helper with the same fallback
```

These changes make it easy to start the main menu without manually
setting PYTHONPATH in most interactive workflows.
...
- Register files with the in-memory session and then run against the registered dataset ids:
  ...
  - Register files with the session (the demo supports --register):
  ...
  - Inspect registered datasets (open a Python REPL):
Examples:
