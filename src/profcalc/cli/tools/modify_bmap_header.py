...
def execute_from_menu():
    ...
    print("=" * 60)
    print("Edit profile headers (name, date, description, purpose) in a BMAP free format file.")
    ...
    for idx, prof in enumerate(profiles, 1):
        print(f"  {idx}. {prof.name} (Date: {getattr(prof, 'date', None) or '-'}; Desc: {getattr(prof, 'description', None) or '-'})")
    ...
    def execute_from_menu():
        ...
        date_str = input("Enter a date to use for all profiles (or leave blank): ").strip()
        purpose_str = input("Enter a purpose or description for all profiles (or leave blank): ").strip()
        ...
        for _ in orig_headers:
            editable_headers.append({
                'date': date_str,
                'purpose': purpose_str
            })

        def curses_header_editor(stdscr):
            import curses
            import re
            curses.curs_set(1)
            stdscr.clear()
            ...
            dirty = False
            help_line = (
                "Arrows: Move  Tab: Switch Field  PgUp/PgDn: Page  D: Set All Dates  P: Set All Purpose  g: Go to  /: Search  Enter: Edit  F2: Save  Esc: Cancel  h: Help"
            )
            show_help = False
            ...
            while True:
                ...
                for idx in range(window_size):
                    ...
                    if i >= n:
                        break
                    stdscr.addstr(idx+2, left_col_x, orig_headers[i][:date_col_x-left_col_x-1])
                    # Highlight current row/field
                    if i == current and field == 0:
                        stdscr.attron(curses.A_REVERSE)
                        stdscr.addstr(idx+2, date_col_x, (edits[i]['date'] or '')[:purpose_col_x-date_col_x-1])
                        stdscr.attroff(curses.A_REVERSE)
                    else:
                        stdscr.addstr(idx+2, date_col_x, (edits[i]['date'] or '')[:purpose_col_x-date_col_x-1])
                    if i == current and field == 1:
                        stdscr.attron(curses.A_REVERSE)
                        stdscr.addstr(idx+2, purpose_col_x, (edits[i]['purpose'] or '')[:max_x-purpose_col_x-1])
                        stdscr.attroff(curses.A_REVERSE)
                    else:
                        stdscr.addstr(idx+2, purpose_col_x, (edits[i]['purpose'] or '')[:max_x-purpose_col_x-1])
                # Show help or main instructions
                if show_help:
                    stdscr.addstr(window_size+4, left_col_x, "KEYS: ↑/k: Up  ↓/j: Down  Tab: Switch Field  PgUp/PgDn: Page  D: Set All Dates  P: Set All Purpose  g: Go to  /: Search  Enter: Edit  F2: Save  Esc: Cancel  h: Toggle Help", curses.A_DIM)
                    stdscr.addstr(window_size+5, left_col_x, "Edit a field, then press F2 to save or Esc to cancel. Unsaved changes will prompt.", curses.A_DIM)
                else:
                    stdscr.addstr(window_size+4, left_col_x, help_line, curses.A_DIM)
                if dirty:
                    stdscr.addstr(window_size+5, left_col_x, "* Unsaved changes", curses.A_BOLD)
                # Move cursor to current field
                if field == 0:
                    stdscr.move((current-scroll)+2, date_col_x + len(edits[current]['date'] or ''))
                else:
                    stdscr.move((current-scroll)+2, purpose_col_x + len(edits[current]['purpose'] or ''))
                stdscr.refresh()
                ...
                if ch in (curses.KEY_UP, ord('k')):
                    ...
                    if field == 0:
                        ...
                        if validate_date(new_val):
                            ...
                            stdscr.addstr(window_size+6, left_col_x, "Invalid date format! Use DDMMMYYYY (e.g., 07NOV2025) or blank.", curses.A_BOLD)
                            stdscr.refresh()
                            stdscr.getch()
                    else:
                        ...
                        if validate_purpose(new_val):
                            ...
                            stdscr.addstr(window_size+6, left_col_x, "Invalid purpose: only letters, digits, spaces, and underscores are allowed.", curses.A_BOLD)
                            stdscr.refresh()
                            stdscr.getch()
                elif ch == 27:  # ESC
                    if dirty:
                    ...
                    if query:
                        ...
                        for i in range(n):
                            idx = (start + i) % n
                            if query.lower() in (edits[idx]['date'] or '').lower() or \
                               query.lower() in (edits[idx]['purpose'] or '').lower() or \
                               query.lower() in orig_headers[idx].lower():
                                found = idx
                                break
        ...
        table_file = out_file.replace('.bmap', '_table.txt') if out_file.endswith('.bmap') else out_file + '_table.txt'
