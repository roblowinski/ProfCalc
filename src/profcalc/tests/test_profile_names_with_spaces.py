...
def test_profile_name_with_spaces_xyz_to_bmap():
    ...
    assert profiles[0].name == 'OC 117', f"Expected 'OC 117', got '{profiles[0].name}'"
    assert profiles[1].name == 'NC 45A', f"Expected 'NC 45A', got '{profiles[1].name}'"
    assert len(profiles[0].x) == 3, f"Expected 3 points for OC 117, got {len(profiles[0].x)}"
    assert len(profiles[1].x) == 2, f"Expected 2 points for NC 45A, got {len(profiles[1].x)}"
...
def test_profile_name_with_date():
    ...
    print("✅ Profile names with dates handled correctly (date extracted to separate field)")
...
def test_bmap_with_spaces_in_header():
    ...
    try:
        assert profiles[0].name == 'OC 117 15AUG2020' or profiles[0].name == 'OC 117' or profiles[0].name == 'OC', \
