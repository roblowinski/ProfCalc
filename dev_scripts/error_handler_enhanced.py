...
# - ENHANCED: Generic array validation utilities
# - ENHANCED: Validation runner patterns
# - ENHANCED: Safe file operation wrappers
# - ENHANCED: Context manager for operation logging
#
# LOGGING SYSTEM:
...
def safe_file_operation(operation_func: Callable, file_path, *args, **kwargs):
    ...
    Raises:
        BeachProfileError: If the file operation fails
    """
    try:
...
def log_operation(logger: StructuredLogger, operation: str, **context):
    ...
    def _log_operation_context():
        ...
        try:
            ...
            logger.info(f"[{operation_id}] {operation} completed successfully in {duration:.3f}s")
        except Exception as e:
            duration = time.time() - start_time
            logger.error(f"[{operation_id}] {operation} failed after {duration:.3f}s: {e}")
            raise
